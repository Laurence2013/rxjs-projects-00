/*
	...
	desc-08: Part 4, using concatMap()
	goal:
	line-code-added:
*/
...
const marketSentiment$ = interval(2000).pipe(map(_ => Math.random() < 0.5 ? 'positive' : 'negative'));
const economicIndicators$ = of({ gdpGrowth: 2.3, inflationRate: 1.8 });

const source00$ = combineLatest([updateStreams03$, marketSentiment$, economicIndicators$], (updates, sentiment, indicators) => {
	return {
		...updates,
		sentiment, 
		indicators
